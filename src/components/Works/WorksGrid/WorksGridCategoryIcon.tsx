export default function WorksGridCategoryIcon({ icon, category, theme, index, isMobile }: { icon: string, category: string, theme: string, index: number, isMobile: boolean | null }) {
    return (
        <div className={`group/categoryicon ${theme === "dark" ? "invert" : ""} relative ${index === 0 ? "w-[38px] h-[38px]" : "w-[24px] h-[24px]"} border-[1px] border-white rounded-full mr-[-5px] flex items-center justify-center bg-[#CBCBCB]`}>
            <img alt="category icon" src={icon} className={`
                ${(index === 0 && category === "UI/UX") ? "scale-90 translate-x-[2px] translate-y-[1px]" : "" } 
                ${(index === 0 && category === "Web") ? "translate-x-[0.5px]" : ""}
                ${(index === 0 && category === "Graphics") ? "translate-x-[1px] scale-95" : ""}
                ${(index === 0 && category === "Product") ? "translate-x-[1px] scale-95" : ""}
                ${index === 0 ? "w-[22px]" : "w-[12px]"}
                object-cover 
                ${category === "Graphics" && "translate-x-[1px]"} 
                ${category === "UI/UX" && "translate-x-[1px] translate-y-[0.2px] scale-95"}`} />
            {!isMobile && (
