...
export default function FolderButton({ isExternalHover }: { isExternalHover?: boolean }) {
    ...
    return (
        <div data-gsap="folder-button" className="group relative w-[120px] h-[90px] [perspective:500px] overflow-visible cursor-pointer">
            <img src="/assets/folder/folder_back.png" className={`w-[120px] h-[90px] object-contain group-hover:translate-y-[2px] transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] ${hoverClass}`} />
            ...
            <img src="/assets/folder/folder_front.png" className={`absolute top-2 left-1/2 -translate-x-1/2 min-w-[125px] min-h-full object-contain origin-bottom transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] group-hover:[transform:translateX(-50%)_rotateX(-30deg)] ${hoverClassFront}`} />
        </div>
    );
}
